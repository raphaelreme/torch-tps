...
class ThinPlateSpline:
    ...
    NOTE: In the classical 2 dimensional TPS, the spline order is 2.
    ...
    def fit(self, X: torch.Tensor, Y: torch.Tensor) -> ThinPlateSpline:
        ...
        if X.shape[0] != Y.shape[0]:
            raise ValueError(f"X and Y should have the same number of points ({X.shape[0]} != {Y.shape[0]})")
        ...
        self.parameters = torch.linalg.solve(A, B)
        self._fitted = True
...
def _ensure_2d(tensor: torch.Tensor) -> torch.Tensor:
    ...
    if tensor.ndim not in (1, 2):
        raise ValueError(f"Only supports 1d and 2d arrays. Found {tensor.ndim} dimensions.")
